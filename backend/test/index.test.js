...
describe("Boat Endpoints", function () {
  ...
  it("POST adds boat. Returns 204.", async () => {
    ...
    // await requestWithSupertest.post("/api/boats", boat) .expect(204)

    await requestWithSupertest.get("/api/boats/boat_name/McBoatFace")
      .then( response => {
  ...
  it("GET api/boats returns a list of boats", async () => {
    ...
    const journey = {"boat_name":"The Journey"}
    const flow = {"boat_name":"Flow"}
     await requestWithSupertest.get("/api/boats")
       .expect(200)
       .then(response => {
         expect(response.type).toEqual("application/json")
         expect(response.body.includes(journey))
         expect(response.body.includes(flow))
       })
  })

  afterEach(() => {
    mongoose.connection.close()
    requestWithSupertest.end
  })
})
